...
from models.PatchTST_lightweight import PatchTST_DualStream_Lightweight
...
def main():
    ...
    model = PatchTST_DualStream_Lightweight(
        cnn_dim=2048,
        flow_dim=179,
        seq_len=15,
        patch_len=5,
        stride=5,
        d_model=64,   # Ultra-small
        n_heads=4,
        n_layers=2,
        dropout=0.1
    ).to(device)
    ...
    for epoch in range(100):
        ...
        if val_r2 > best_val_r2:
            ...
            torch.save(model.state_dict(), "results/best_PatchTST.pth")
        else:
            patience_counter += 1
    ...
    model.load_state_dict(torch.load('results/best_PatchTST.pth'))
    ...
    import json
    with open("results/PatchTST_results.json", "w") as f:
        json.dump({
            "model": "PatchTST",
    ...
    print("✅ 结果已保存至: results/PatchTST_results.json")
