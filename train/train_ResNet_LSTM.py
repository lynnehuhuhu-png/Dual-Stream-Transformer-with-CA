...
from models.ResNet_LSTM_adapted import ResNetLSTM_DualStream
...
def evaluate(model, loader, device):
    ...
    with torch.no_grad():
        for cnn_x, flow_x, y in loader:
            ...
            preds.extend(pred.float().cpu().numpy().tolist() if pred.dim() > 0 else [pred.float().cpu().item()])
...
def print_rainfall_level_results(results):
    """以表格形式打印分级评估结果"""
    print("\n" + "=" * 80)
    print("Rainfall Level Evaluation Results")
    print("=" * 80)
...
def train_epoch(model, loader, optimizer, scaler, device, accumulation_steps=4):
    ...
    for batch_idx, (cnn_x, flow_x, y) in enumerate(tqdm(loader, desc="Training", leave=False)):
        ...
        preds.extend(pred.detach().float().cpu().numpy().tolist() if pred.dim() > 0 else [pred.detach().float().cpu().item()])
        targets.extend(y.cpu().numpy().tolist() if y.dim() > 0 else [y.cpu().item()])

    return total_loss / len(loader), r2_score(np.array(targets), np.array(preds))

def main():
    print("="*80 + "\nResNet-LSTM Comparison Experiment\n" + "="*80)
    ...
    # Create model (ultra-lightweight with gradient checkpointing)
    model = ResNetLSTM_DualStream(
        cnn_dim=2048,
        flow_dim=179,
        seq_len=15,
        hidden_dim=256,
    ...
    print(f"Training started\n" + "="*80 + f"\n{'Epoch':<8} | {'Train R2':<10} | {'Val R2':<10} | {'Val MAE':<10} | {'Val RMSE':<10} | {'Best':<6} | {'Patience':<8}\n" + "-"*80)
    ...
    for epoch in range(100):
        ...
        if val_r2 > best_val_r2:
            ...
            torch.save(model.state_dict(), "results/best_ResNet-LSTM.pth")
        else:
            patience_counter += 1
    ...
    print("="*80 + f"\nTraining completed!\n   Best Val R2: {best_val_r2:.4f} (Epoch {best_epoch})\n   Best Val MAE: {best_val_mae:.4f}\n   Best Val RMSE: {best_val_rmse:.4f}\n" + "="*80)

    # 加载最佳模型进行分级评估
    print("\n" + "=" * 80)
    print("Loading best model for level evaluation...")
    print("=" * 80)

    model.load_state_dict(torch.load('results/best_ResNet-LSTM.pth'))
    ...
    import json
    with open("results/ResNet-LSTM_results.json", "w") as f:
        json.dump({
            "model": "ResNet-LSTM",
    ...
    print("Results saved to: results/ResNet-LSTM_results.json")
